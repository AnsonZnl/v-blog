...
开发微信小程序中，经常会用到获取一些用户权限的页面，比如你要登录，就要获取个人信息、你要做人脸识别，就要获取相机权限、你要做位置地图功能、就要获取用户的位置权限...
...
写一套写下来，在掺杂着业务逻辑，那真的是..要多恶心..有多恶心啊~
...
```html
<!-- index.wxml -->
<view>是否授权：{{isAuth}}</view>
<camera wx:if="{{isAuth}}" style="width: 100%; height: 300px;"></camera>
```
